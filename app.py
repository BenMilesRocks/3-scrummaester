...
def register():
    ...
    if form.validate_on_submit():
        hashed_password = bcrypt.hashpw(
            form.password.data.encode('utf8'), bcrypt.gensalt()
            )
        new_user = User(
...
def add_project():
    ...
    return render_template(
        "add_project.html", teams=team_db,
...
def update_project(project_id):
    ...
    if request.method == "POST":
        project = update(Project).where(
            Project.project_id == project_id
            ).execution_options(populate_existing=True).values(
            project_name=request.form.get("project_name"),
            project_status=request.form.get("project_status"),
            team_id=request.form.get("team_id")
            )
...
def update_current_user():
    ...
    if request.method == "POST":
        hashed_password = bcrypt.hashpw(
            request.form.get("password").encode('utf8'), bcrypt.gensalt()
        )
        user = update(User).where(
...
def add_task():
    ...
    if request.method == "POST":
        if (
            re.search(
                "^[^\s]+(\s+[^\s]+)*$", request.form.get("task_description"))
                ) is None:
            flash(
                """Task desription can't have whitespace at
                the beginning or end of the description. Please try again"""
                )
    ...
    return render_template(
        "add_task.html", projects=project_db,
        users=user_db, teams=team_db,
        previous_url=previous_url
    )
...
def update_task(task_id):
    ...
    if request.method == "POST":
        if (
            ...
            flash(
                """Task desription can't have whitespace at the
                beginning or end of the description. Please try again"""
                )

        # If valid, submit
        else:

            task = update(Task).where(
